...
<head>
    ...
    <body>
       	<div class="container">
            ...
            <hr>
                <form action="<?= base_url('index.php/users/update_grn') ?>" method ="post">
                    <div style="width: 100%;">
                        ...
                        <div style="float:right;width: 40%">
                            <table class='payment'>
                                    ...
                                    <tr><th>Order No.</th><td>:<select name="po_no_year"><option>PO/2015-16/REY/</option></select><?= $r->po_no ?><a href="<?= base_url('index.php/users/print_po/'.$r->po_no); ?>" class="glyphicon glyphicon glyphicon-eye-open" target="blank" title="View PO"></a></td></tr>
                                   
                                <tr><th>Date</th><td>:<input type="text"  name='grn_date' id="datepicker1" Required="required" value="<?= $r->grn_date ?>" style="background:#F5F5DC"/></td></tr>
                                <tr><th>Dc No.</th><td>:<input type="text"  name='dc_no' placeholder="Enter Dc No." value="<?= $r->dc_no ?>"/></td></tr>
                                <tr><th>Dc Date</th><td>:<input type="text"  name='dc_date' id="datepicker2" placeholder="dd-mm-yy" value="<?= $r->dc_date ?>" /></td></tr> 
                            </table>
                        </div>
                    ...
                    <tr class="item-row">
                        ...
                        <td  align="center"><?= $row->work_order ?><input type="hidden" name="work_order[]" value="<?= $row->work_order ?>"></td>
                         <?php if($row->weight==$row->quantity){ ?>
                        <td align="center"><input type="text" name="qty_rec[]" value="<?= $row->qty_rec ?>" size="8" style="background:#F5F5DC" class="qty_rec"></td>
                         <td align="center"><input type="hidden" name="wt_rec[]" value="-" class="wt_rec" size="8">-</td>
                        <?php }else{ ?>
                        <td align="center"><input type="text" name="qty_rec[]" value="<?= $row->qty_rec ?>" class="qty_rec" style="background:#F5F5DC" size="8"></td>
                        <td align="center"><input type="text" name="wt_rec[]" value="<?= $row->wt_rec ?>" class="wt_rec" size="8"></td>
                        <?php } ?>
                        <?php if($row->weight==$row->quantity){ ?>
                        <td align="center"><input type="text" size="8"  name="qty_pen[]" class="qty_pen"  value="<?= $row->qty_pen ?>"></td>
                         <td align="center"><input type="hidden" name="wt_pen[]" class="wt_pen" size="8" value=-">-<input type="hidden" class="value" name="price[]"></td>
                        <?php }else{ ?>
                        <td align="center"><input type="text"  name="qty_pen[]" class="qty_pen" value="<?= $row->qty_pen ?>" size="8"></td>
                        <td align="center"><input type="text" name="wt_pen[]" class="wt_pen" size="8" value="<?= $row->wt_pen ?>"><input type="text" class="value" name="price[]"></td>
                        <?php } ?>
                    </tr>
                            <?php }  ?> 
                    </table>    
